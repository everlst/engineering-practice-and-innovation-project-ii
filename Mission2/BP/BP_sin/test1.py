...
class SimpleNet(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(1, 320)  # 输入层到隐藏层，320个神经元
        self.fc2 = nn.Linear(320, 320)  # 隐藏层到隐藏层
        self.fc3 = nn.Linear(320, 1)  # 隐藏层到输出层
...
# Step 6: 预测并绘制拟合效果
net.eval()  # 预测时设为评估模式
y_pred = net(x_train).detach().numpy()  # 预测结果并转换为numpy数组

# 绘制原始正弦曲线和拟合曲线
plt.plot(x, y, label="True Sine Curve")
plt.plot(x, y_pred, label="Fitted Curve", linestyle="dashed")
plt.legend()
plt.title("Sine Curve Fitting Using BP Neural Network")
plt.show()
