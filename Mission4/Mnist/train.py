...
def main():
    ...
    parser.add_argument(
        "--data_url", type=str, default=False, help="mnist dataset path"
    )
    parser.add_argument("--train_url", type=str, default=False, help="mnist model path")
    ...
    dataset1 = datasets.MNIST(
        args.data_url, train=True, download=False, transform=transform
    )
    dataset2 = datasets.MNIST(
        args.data_url, train=False, download=False, transform=transform
    )
    ...
    if args.save_model:
        ...
        shutil.copyfile(
            os.path.join(the_path_of_current_file, "infer/customize_service.py"),
            os.path.join(model_path, "customize_service.py"),
        )
        shutil.copyfile(
            os.path.join(the_path_of_current_file, "infer/config.json"),
